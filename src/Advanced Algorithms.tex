...
\begin{document}
    ...
    Many important computational problems in computer science are \NPclass-complete, meaning that unless \Pclass = \NPclass, no polynomial-time algorithm can solve all instances of these problems exactly. Given the \tbf{intractability} of exact solutions, especially as problem sizes grow, \tit{approximation algorithms} provide a practical alternative. These algorithms aim to efficiently produce solutions that are \curlyquotes{good enough} --- often within a provable bound of the optimal solution.

    In these notes we will explore the design and analysis of approximation algorithms for various \NPclass-complete problems. We will study a variety of approximation techniques including greedy strategies, randomization, linear programming relaxations, primal-dual methods and metric embeddings. Through these techniques, we will develop algorithms for classic problems such as Max Cut, Vertex Cover, Set Cover, and analyze their approximation guarantees.

    Lastly, \underline{\tit{unless specified}} in these notes, when talking about \tbf{approximation algorithms}, we will adopt the following convention: given a problem P for which there is an optimal solution OPT, and an algorithm that solves P which outputs a solution ALG to P
    ...
    \begin{framedprop}{Limits of LP relaxation}
        Given a problem P for which there is an $\alpha$-approximation algorithm which uses LP relaxation, consider an optimal \tit{rounded} solution $\mbox{ALG}_{\mathrm{P}}$. Then, it holds that:
    ...
    \begin{proof}
        ...
        \claim{
            ...
            This concludes that $n - 1$ vertices are \tit{sufficient} to cover $V(K_n)$.

            Now, by way of contradiction suppose that there is a vertex cover $S'$ of $K_n$ such that $\abs{S'} \le n - 2$. This implies that there are two vertices $u, v \in V(K_n) - S'$. However, this means that the edge $\{u, v\} \in E(K_n)$ is not covered --- and such edge always exists since $K_n$ is a complete graph. This proves that $n - 1$ vertices are \tit{needed}.
        }
    ...
    To fix this issue, we are going to present a \tbf{randomized rounding algorithm}, which surprisingly seems to be the \tit{best} approach that we have at our disposal to perform rounding on LP relaxation solutions of this problem.
    ...
    \begin{framedalgo}{Randomized rounding for SC}
        ...
        \begin{algorithmic}[1]
            \Function{RandomizedRoundingSC}{$\mathcal U$, $C$}
                \State $A := \varnothing$
                \State $\{\overline x_j\}_{j \in [m]} := \mathrm{LP_{SC}}(\mathcal U, C)$ \Comment{an optimal solution on the LP relaxation}
    ...
    By the law of large numbers, we know that if for each pair $(\mathcal U, C)$ we repeatedly compute the previous algorithm we will, eventually, get an output $A'$ such that $\abs{A'} \le \ceil{2 \ln n} \mathrm{IP}^*$. This implies that such procedure yields an approximation ratio $\alpha \le \ceil{2 \ln n}$, implying that $$\ceil{2 \ln n } \ge \alpha \ge \mathrm{IG}_{\mathrm{SC}}$$ directly providing an upper bound for SC.
    ...
    The LP that we will present for this problem is the following. However, to show that this LP actually yields a solution for DS is not trivial, and it will be proven throughout the rest of the chapter.
    ...
    Consider the following generic linear program for a \tit{maximization} problem:
    ...
    First of all, consider the following computational problem.
    ...
    As for the case of \tit{graph density}, we require $S \neq \varnothing$, but we will omit this detail in the following sections. Moreover, note that by definition of \tit{cut}, we have that for any $S \subseteq V(G)$ it holds that $$0 \le \abs{\mathrm{cut}(S)} \le \abs {S \times \overline S} = \abs S \cdot \abs{\overline S}$$ since an edge $e$ is in $\mathrm{cut}(S)$ if and only if $\abs {S \cap e} = 1$, hence the cartesian product $S \times \overline S$ represents the \tit{possible} edges that have exactly one endpoint in $S$. Therefore, we have that $0 \le \psi(S) \le 1$, and in particular
    ...
    A good tool of reasoning for SC is the following type of graphs, introduced by \textcite{erdos} in 1959.
    ...
    But why are we introducing the Sparsest Cut problem in the first place? The Max Cut problem --- discussed in \cref{maxcut} --- can be reduced to SC, implying that the latter is \NPclass-hard as well. However, the problem can be approximated through the \tbf{Leighton-Rao algorithm} \cite{leightonrao}, which relies on LP relaxation and a series of reduction steps. In particular, its integrality gap is bounded using \tit{geometric arguments}, therefore in this chapter we will focus on \tbf{metric geometry} in order to establish this result.
    ...
    \begin{framedthm}{}
        Given $n \in \N$, let $\func{f}{\powerset([n])}{\R}$ be a submodular and non-negative function. Given $\displaystyle S^* \in \argmax_{S \subseteq [n]}{f(S)}$, if $R$ is chosen from $[n]$ UAR, it holds that $$\Exp \sbk{f(R)} \ge \dfrac{1}{4}f(S^*)$$
    \end{framedthm}
    ...
    \begin{proof}
        ...
        On the first round, the adversary sends the edges $\{a, x\}$ and $\{b, x\}$; then, our algorithm will match $x$ to either $a$ or $b$, and clearly it will learn nothing about the vertex $y$ whose edges will be sent by the adversary on the next round --- namely, they can send either $\{b, y\}$ or $\{a, y\}$. Through a similar argument used to prove \cref{mbm det}, we see that the probability of $y$ being impossible to match on the next round is $\tfrac{1}{2}$. This implies that $$\Exp[\mbox{number of matched edges}] = 1 \cdot \dfrac{1}{2} + 2 \cdot \dfrac{1}{2} = \dfrac{3}{2}$$ However, an offline deterministic algorithm (which is a also randomized algorithm that makes no random choices) can always retun an optimal solution with 2 matched edges, therefore the expected ratio is $$\dfrac{\tfrac{3}{2}}{2} = \dfrac{3}{4}$$
    \end{proof}
    ...
    \begin{framedthm}{}
        Given a set trained experts $M$, for every deterministic algorithm based on the expert model the total number of mistakes $m$ is such that $m \ge \log n$ and $m \ge 2{m^*}$, where $m^*$ is the number of mistakes of the best expert after $T$ rounds.
    \end{framedthm}
    ...
    To fix this problem, we need a payoff scheme based on \tbf{scoring rules}. Scoring rules act as a generalization of the payoff scheme, where the gain and the payoff do not have to add up to 1. In other words, if the forecaster guesses $x \in [0, 1]$ as the probability of the event, then it gains $f_1(x)$ points if the event really happens, otherwise it loses $f_0(x)$ points --- in the previous scheme, $f_1(x) = x$ and $f_0(x) = 1- x$. Hence, the expected gain now becomes $$g_\mu(x) = \Exp_\mu[P_x] = f_1(x) \mu + f_0(x)(1 - \mu)$$ An optimal scoring rule would be one that makes the model converge to $x = \mu$.
        ...
        \begin{algorithmic}[1]
            \Function{DFS-2-coloring}{$G$, $u$, \ttt{col}, $c$}
                ...
                \For{$v \in \mathcal N(u)$}
                    \If{$\textsc{DFS-2-coloring}(G, v, \ttt{col}, 1 - c)$ == \ttt{False}}
                        \State \tbf{return} \ttt{False}
                    \EndIf
                \EndFor
            ...
            \Function{2-coloring}{$G$}
                ...
                \For{$u \in V(G)$}
                    \If{$\ttt{col[}u\ttt{]} == -1$}
                        \If{$\textsc{DFS-2-coloring}(G, u, \ttt{col}, 0)$ == \ttt{False}}
                            \State \tbf{return} \ttt{False}
                        \EndIf
                    \EndIf
                \EndFor
    ...
    \solution{
        ...
        \begin{itemize}
            \item initially, the algorithm will remove all the vertices of the biggest partition of $K_{t, 2^t}$ one by one, since they all have degree $t$, and then the whole bipartite graph will be removed such that only the $K_{t + 2}$-cliques remain
    ...
    \solution{
        ...
        \begin{enumerate}
            ...
            \item We observe that in the previous exercise we did not use the fact that $f(\varnothing) = 0$, but we are going to leverage this property now. Consider a modular function $f$ that can be queried as described in the statement; through the claim of the previous exercise we proved that $f$ is learnable if and only if $f(\varnothing), f(\{1\}), \ldots, f(\{n\})$ are learnable. Thus, since $f(\varnothing)$ is given, we need to determine a general condition which allows us to find the values for $f(\{1\}), \ldots, f(\{n\})$ given only $f(\{i, j\})$ for each $\{i, j\} \in E(G)$.
                ...
                \claim{
                    For each component $H$ of $G$, it holds that $A^{H}x = b$ has unique solution if and only if $H$ is not bipartite.
                }{
                    Fix a component $H$, and let $n_{H} := \abs{V(H)}$; through linear algebra we know that $A^{H}x = b$ has unique solution if and only if the rank of the coefficient matrix is equal to the number of unknowns, which is $n_{H}$. Given a matrix $A$, the rank-nullity theorem states that $$\mathrm{rank}(A) + \dim(\ker A) = n$$ where $n$ is the number of \tit{columns} of $A$. Therefore, in our case it holds that $$\mathrm{rank}(A^{H}) + \dim(\ker {A^{H}}) = n_{H}$$ Hence, to prove that $\mathrm{rank}(A^{H}) = n_{H}$ it suffices to show that $\dim(\ker {A^{H}}) = 0$. This means that to prove our statement we have to show that the dimension of the kernel is 0 if and only if $H$ is not bipartite.

                    Assume that $H$ is not bipartite, and by way of contradiction suppose that there exists $\bar x = [f(v_1) \ \cdots \ f(v_{n_{H}})]^T \in \ker {A^{H}}$ such that $\bar x \neq 0$. By definition, since $\bar x$ is in the kernel of ${A^{H}}$, it holds that $A^{H} \bar x = 0$, and by construction of $A^{H}$ this happens if and only if for each edge $\{i, j\} \in E(H)$ it holds that $$f(\{i\}) + f(\{j\}) = 0 \iff f(\{i\}) = - f(\{j\})$$ This means that the values assigned to adjacent vertices \tit{are always the same but opposite in sign}: in other words, the vertices of $V(H)$ can be 2-colored such that
                    ...
                    and since a graph is 2-colorable if and only if it is bipartite, this contradicts the assumption for which $H$ was not bipartite. This implies that all the $x \in \ker {A^{H}}$ are such that $x = 0$, i.e. the kernel has dimension 0.
                    
                    Vice versa, assume that $H$ is bipartite through a bipartition $(A, B)$, and consider the following vector $\hat x \in \R^{n_{H} \times 1}$ $$\forall i \in V(H) \quad \hat x(i) = \soe{ll}{1 & i \in A \\ -1 & i \in B}$$ Now consider the matrix product $A^{H} \hat x$: by definition of $A^{H}$ and $\hat x$, the $k$-th equation of the corresponding system of equations will be equal to $x_i + x_j$ where $\{i, j\}$ is the $k$-th edge of $E(H)$. Now, since $H$ is bipartite, we can assume wihtout loss of generality that $i \in A$ and $j \in B$, which implies that $$x_i + x_j = 1 + (-1) = 0$$ Finally, by applying the same argument on all the edges of $E(H)$ we have that $A^{H} \hat x = 0$, i.e. $\hat x \in \ker {A^{H}}$. However, by definition it holds that $\hat x \neq 0$, which implies that the kernel contains non-zero vectors, implying that $\dim(\ker {A^{H}})) > 0$.
                }

                Let $S_G$ be the solution space of $A^Gx = b$. Similarly, for each $i \in [t]$ let $S_{H}$ be the solution space of $A^{H} x = b$. Clearly, it holds that $S_G = S_{H_1} \cap \ldots \cap S_{H_t}$. Hence, by the claim above we conclude that $S_G$ has a unique solution if and only if there is no bipartite component of $G$. Therefore, it holds that $f$ is learnable if and only if every component of $G$ has at least one odd-length cycle.
    ...
    \begin{framedalgo}[breakable]{The $f$-Generalized W.M. Expert model}
        Given a set $M$ of $n$ trained experts, the algorithm yields predictions over time $T$. \\
        \hrule
    ...
    \begin{framedprob}{}
        ...
        Prove that, if $f(w_i) = \tfrac{1}{w_i + 1}$, then the number of mistakes of the $f$-Generalized Weighted Majority algorithm is $\Omega(n)$ even in cases where $m^* = 0$.
    ...
    \solution{
        ...
        In fact, an \tit{adversarial argument} suffices to prove such lower bound on $m$, and the answer lies in how the updating rule operates. In particular, starting from $w_i = 1$, we get that each time the $i$-th expert is wrong $w_i$ is transformed as follows: $$1 \stackrel{f}{\longrightarrow} \dfrac{1}{2} \stackrel{f}{\longrightarrow} \dfrac{1}{3} \stackrel{f}{\longrightarrow} \cdots$$ which is precisely the harmonic sequence. In other words, after $k$ mistakes of the $i$-th expert, it holds that $w_i = h_k = \tfrac{1}{k}$, where $h_k$ is the $k$-th member of the harmonic sequence.
    ...
    \begin{framedprob}{}
        Consider an urn containing $n$ balls, the $i$-th ball of which labelled by the integer $i$. You can sample the urn as follows: you draw a UAR ball, look at its label, and then put the ball back into the urn. Give an upper bound on the number of samples needed to see each of the $n$ labels at least once, with probability at least $\tfrac{9}{10}$.
    \end{framedprob}

    \solution{
        We can rewrite the problem thorugh the following algorithm:

        \begin{algorithmic}[1]
            \Function{BallExtraction}{$T$}
                \State $A := \varnothing$
                \For{$t \in [T]$}
                    \State Sample $x_t \in [n]$ UAR
                    \State $A = A \cup \{x_t\}$
                \EndFor
                \State \tbf{return} $A$
            \EndFunction
        ...
        Given this procedure, the problem asks to find a bound for the number of iterations $T$ such that $A := \textsc{BallExtraction}(T)$ is such that $$\Pr \sbk{\abs A = n} \ge \dfrac{9}{10}$$ We observe that
        \begin{equation*}
            \begin{alignedat}{2}
                \Pr \sbk{\abs A = n} &= 1 - \Pr \sbk{\abs A < n} && \\
                                     &= 1 - \Pr \sbk{\exists i \in [n] \quad i \notin A} && \\
                                     &\ge 1 - \sum_{i = 1}^n{\Pr[i \notin A]} && \quad \quad (\mbox{by the union bound}) \\
                                     &= 1 - \sum_{i = 1}^n{\Pr[\forall t \in [T] \quad x_t \neq i]} && \\
                                     &= 1 - \sum_{i = 1}^n{\prod_{t = 1}^T{\Pr[x_t \neq i]}} && \\
                                     &= 1 - \sum_{i = 1}^n{\prod_{t = 1}^T{\rbk{\dfrac{n- 1}{n}}}} && \\
                                     &= 1 - \sum_{i = 1}^n{\rbk{\dfrac{n - 1}{n}}^T} && \\
                                     &= 1 - n \rbk{\dfrac{n - 1}{n}}^T && \\
            \end{alignedat}
        \end{equation*}
        Now, we observe that
        \begin{equation*}
            \begin{split}
                     & 1 - n \rbk{\dfrac{n - 1}{n}}^T \ge \dfrac{9}{10} \\
                \iff & n \rbk{\dfrac{n - 1}{n}}^T \le 1 - \dfrac{9}{10} = \dfrac{1}{10} \\
                \iff & \rbk{\dfrac{n - 1}{n}}^T \le \dfrac{1}{10n} \\
            \end{split}
        \end{equation*}
        Moreover, assuming that $n > 1$ we can simplify even more as follows $$T \ge \log_{\tfrac{n - 1}{n}}{\dfrac{1}{10n}} = \dfrac{\ln \rbk{\tfrac{1}{10n}}}{\ln \rbk{\tfrac{n - 1}{n}}} = \dfrac{\ln \rbk{\tfrac{1}{10n}}}{\ln \rbk{1 - \tfrac{1}{n}}}$$ and by using the fact that for all $n > 1$ it holds that $-\dfrac{1}{n} \ge \ln \rbk{1 - \dfrac{1}{n}}$ we get that $$T \ge \dfrac{\ln \rbk{\tfrac{1}{10n}}}{\ln \rbk{1 - \tfrac{1}{n}}} \ge -n \cdot \ln \rbk{\dfrac{1}{10n}} = n \ln(10n)$$ Finally, this implies that $$T \ge n \ln (10n) \implies \Pr \sbk{\abs A = n} \ge 1 - n \rbk{\dfrac{n - 1}{n}}^T \ge \dfrac{9}{10}$$

        We observe that this is an \tit{upper bound} in the probabilistic sense: this bound on $T$ tells that with probability at least $\tfrac{9}{10}$ the number of extractions required to see all $n$ balls is at most $T = n \ln (10n)$. In other words, this is an upper bound on the number of extractions with 90\% confidence.
    }
