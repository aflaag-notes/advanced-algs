...
\begin{document}
    ...
    \begin{frameddefn}{Cut}
        Given an undirected graph $G = (V, E)$, and a subset of its vertices $S \subseteq V$, the \tbf{cut} induced by $S$ on $G$ is defined as follows $$\mathrm{cut}(S) := \{e \in E : \abs{S \cap e} = 1\}$$
    \end{frameddefn}
    ...
    Even though this approach yields $\tfrac{1}{2}$-approximation algorithm that runs in only $n$ iterations, for very large graphs this is still too slow. Is there a way to improve our algorithm? Observe how \textsc{Charikar} works: at each iteration, the algorithm removes a vertex of minimum degree of the current graph. What if we remove \tit{all} the vertices of minimum degree in the graph all at once? \todo{controlla con chatgpt grammatica di sto par}
    ...
    As for the case of \tit{graph density}, we require $S \neq \varnothing$, but we will omit this detail in the following sections. Moreover, note that by definition of \tit{cut}, we have that for any $S \subseteq V(G)$ it holds that $$0 \le \abs{cut} \le \abs {S \times \overline S} = \abs S \cdot \abs{\overline S}$$ since an edge $e$ is in $\mathrm{cut}(S)$ if and only if $\abs {S \cap e} = 1$, hence the cartesian product $S \times \overline S$ represents the edges that have exactly one endpoint in $S$. Therefore, we have that $0 \le \psi(S) \le 1$, and in particular
    ...
    \begin{frameddefn}{Cut metric}
        Given a graph $G$, a sequence $d_{T_1}, \ldots, d_{T_k}$ of elementary cut metrics on $G$, and $\lambda_1, \ldots, \lambda_k > 0$ positive real values, a \tbf{cut metric} is a function defined as follows $$\funcmap{d}{V(G) \times V(G)}{\R}{(x, y)}{\sum_{i \in [k]}{\lambda_i d_{T_i}(x, y)}}$$
    \end{frameddefn}
    ...
    TODO \todo{prev cor, sure about last equality?}
    ...
    \begin{frameddefn}{$\ell_\mu$ metrics}
        Given $\mu \in \R_{\ge 1}$, and a set $S \subseteq \R^d$ for some $d$, the \tbf{$\ell_\mu$ metric} is a function defined as follows $$\ell_\mu(x, y) = \sqrt[\mu]{\sum_{i = 1}^d{\abs{x_i - y_i}^\mu}}$$
    \end{frameddefn}
    ...
    TODO \todo{bruteforce thing? + interlude}
    ...
    \begin{framedalgo}{$\rbk{1 - \frac{1}{e}}$-approximation for MC}
        ...
        \quad
        \begin{algorithmic}[1]
            \Function{ApproximatedMaxCover}{$\mathcal U$, $C$, $k$}
                \State $S := \varnothing$
    ...
    \begin{framedthm}{}
        The \textsc{ApproximatedMaxCover} algorithm returns a $\rbk{1 - \tfrac{1}{e}}$-approximation of MC.
    \end{framedthm}
    ...
    \begin{proof}
        ...
        Let $S := \textsc{ApproximatedMaxCover}(\mathcal U, C, k)$; hence, we have that $T_i = \bigcup_{j = 1}^i{S_j}$. Additionally, let $t_i := \abs{T_i}$ and $\mu_i := \abs{X^*} - t_i$. Hence, we have that
        ...
        \claim{
            ...
            We will prove the claim by strong induction on $i$. In particular, if $i = 0$, we have that $\mu_0 = \abs{X^*} \rbk{1 - \dfrac{1}{k}}^0 \abs{X^*} = 1 \cdot \abs{X^*}$, hence the base case trivially holds. Then, assuming the strong inductive hypothesis, we can prove the inductive step as follows \todo{non capisco (TODO dentro allo split)}
            \begin{equation*}
                \begin{split}
                    \mu_{i + 1} &= \abs{X^*} - t_{i + 1} \\
                                ...
                                &\le \rbk{1 - \dfrac{1}{k}} \rbk{1 - \dfrac{1}{k}}^i \abs{X^*} \quad \quad (\mathrm{by \ strong \ inductive \ hypothesis \ on} \ \mu_i) \\
