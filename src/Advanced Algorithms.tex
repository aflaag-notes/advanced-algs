...
\begin{document}
    ...
    \begin{itemize}
        \item if $P$ is a \tbf{minimization} problem, we say that $A$ yields an $\alpha$-approximation of $P$ if $$\mathrm{ALG} \ge \alpha \cdot \mathrm{OPT}$$
        \item if $P$ is a \tbf{maximization} problem, we say that $A$ yields an $\alpha$-approximation of $P$ if $$\mathrm{ALG} \le \alpha \cdot \mathrm{OPT}$$
    \end{itemize}
