...
\begin{document}
    ...
    \begin{frameddefn}{Isometrical embedding}
        Consider two metrics $\func{d_1}{A}{B}$ and $\func{d_2}{X}{Y}$; we say that $d_1$ is \tbf{isometricaly embedded} into $d_2$ if there is a function $\func{f}{S}{X}$ such that $$d_1(x, y) = d_2(f(x), f(y))$$
    \end{frameddefn}

    TODO \todo{prev def non ha senso}
    ...
    \begin{proof}
        ...
        This means that for any $i \in [k]$ and $u, v \in V(G)$, it holds that $$\1 \sbk{\abs{\{u, v\} \cap T_i}} = \abs{\overline{x_u}(i)- \overline{x_v}(i)}$$ Therefore, we obtain the following
        \begin{equation*}
            \begin{split}
    ...
    \begin{framedlem}{}
        For any $d$, and any set $X \underset{fin}{\subseteq} \R^d$, the $\ell_1$ metric over $X$ is isometrically embedded into a cut metric defined over $k = d \cdot(\abs x - 1)$ cuts.
    \end{framedlem}
