...
\begin{document}
    ...
    In TODO \cref{maxcut} we mentioned that the best known approximation ratio for the Max-Cut problem is around 0.878. We are now ready to discuss this ratio.
    ...
    Despite many efforts, for a very long time researchers believed that no improvement over this result could be achieved. However, in a groundbreaking paper published in 1995 \textcite{goemans} proved everyone wrong, by completely changing the approach: instead of using an IP, they modeled the Max-Cut problem through a \tbf{Quadratic Program} (QP).
    ...
    This concludes that Goemans and Williamson's algorithm is \tit{the best} rounding algorithm that can be achieved through SDPs. Moreover, researchers believe that this approximation is the best that can be achieved in general for MC. In particular, the main reasong behind such hypothesis is the \tbf{Unique Games Conjecture}, which will be discussed in the following section.
    ...
    \begin{proof}
        ...
        \claim[Claim 1]{
            ...
            \begin{equation*}
                ...
                \end{split}
            \end{equation*}
            TODO \todo{recupera max cover prima di finire}
        }
    ...
    This algorithms is identical to the naïve approach except for a very important detail: instead of choosing the $i$-th expert UAR, this approach uses a \tbf{majority voting system}. In fact, since $z_t$ is the majority of the predictions in $y_t$, and the values of the outputs are either 0 or 1, at each mistake now \tit{at least half} of the experts must be wrong, thus discarded. Hence, we get the following proposition.
    ...
    Both of these algorithms rely on the guarantee that \tit{at least one} perfect model is present in the input, but what there is no such model? In this case, the algorithm will clearly discard all the experts eventually. Consider the following procedure.
    ...
    \begin{proof}
        ...
        \claim{
            On each round $t \in [T]$, if $z_t \neq x_t$ then $\tfrac{3}{4}W^{(t)} \ge W^{(t  + 1)}$.
        }{
            Let $I^{(t)}$ be the total weight of the wrong experts at the beginning of round $t$, i.e. $$I^{(t)} := \sum_{\substack{i \in [n]} : \\ y_{i, t} \neq x_t}{w_i^{(t)}}$$ If $z_t \neq x_t$ the weight of at least half of the experts will get halved, hence $I^{(t)} \ge \tfrac{1}{2}W^{(t)}$. Thus, we have that $$W^{(t + 1)} = \dfrac{I^{(t)}}{2} + (W^{(t)} - I^{(t)}) = W^{(t)} - \dfrac{I^{(t)}}{2} \le W^{(t)}- \dfrac{W^{(t)}}{4} = \dfrac{3}{4}W^{(t)}$$
        }
    ...
    % \begin{equation*}
        \begin{split}
            \Exp[\max(X_1, X_2)] &= \Pr \sbk{X_2 = \dfrac{4}{3}} + \Pr \sbk{X_2 = \dfrac{1}{3}} \cdot \Pr[X_1 = 1] \cdot 1 + \Pr \sbk{X_2 = \dfrac{1}{3}} \cdot \Pr[X_1 = 0] \cdot \dfrac{1}{3} \\
    ...
    \solution{
        ...
        \claim{
            ...
            Let $S$ be a set such that $\abs{\mathrm{cut}(S, \overline S)} = \abs{E(G)}$, and consider the two sets $(S, \overline S)$; then, by definition it holds that $$\mathrm{cut}(S, \overline S) = \{uv \mid u \in S \land v \in \overline S\}$$ which implies that $(S, \overline S)$ is a bipartition of $G$.
