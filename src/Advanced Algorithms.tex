...
\begin{document}
    ...
    \solution{
        ...
        \claim{
            ...
            Since the values inside the matrix $G$ are \texteuro's, $g_c$ is trivially non-negative.
            ...
            \begin{itemize}
                ...
                \item If $S \cap T = \varnothing$, let $M_S := g_c(S)$ and $M_T := g_c(T)$; we observe that clearly $g_c(S \cup T) = \max{\{M_S, M_T\}}$, therefore it holds that
                    \begin{equation*}
                        \begin{split}
                            g_c(S) + g_c(T) &= \max_{f \in S}{G[c][f]} + \max_{f \in T}{G[c][f]} \\
                                            &\ge \max_{f \in S \cup T}{G[c][f]} + 0 \\
