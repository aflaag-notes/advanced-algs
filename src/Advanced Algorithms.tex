        ...
        \begin{enumerate}
            ...
            \item We observe that in the previous exercise we did not use the fact that $f(\varnothing) = 0$, but we are going to leverage this property now. Consider a modular function $f$ that can be queried as described in the statement; through the claim of the previous exercise we proved that $f$ is learnable if and only if $f(\varnothing), f(1), \ldots, f(n)$ are learnable. Thus, since $f(\varnothing)$ is given, we need to determine a general condition which allows us to find the values for $f(1), \ldots, f(n)$ given only $f(\{i, j\})$ for each $\{i, j\} \in E(G)$.
                ...
                \claim{
                    ...
                    % We know that $A^{H_i}x = b$ has at least one solution if and only if if has at least as many linearly independent rows as linearly independent columns, which is equal to $\abs{V(H_i)}$ by construction of $A^{H_i}$. Moreover, each non-empty row of $A^{H_i}$ is already linearly independent from the other ones by construction.
                    % We know that $A^{H_i}x = b$ has unique solution if and only if the rank of the matrix is equal to the number of unknowns, which is going to be $\abs{V(H_i)}$. Hence, let $H_i$ be an odd-length cycle component, and consider the resulting coefficient matrix $A^{H_i}$: $$\smat{1 & 1 & 0 & \ldots & 0 \\ 0 & 1 & 1 & \ldots & 0 \\ & & \vdots & & \\ 1 & 0 & 0 & \ldots & 1}$$ By construction, this matrix will have $\abs{V(H_i)}$ rows, and through Gaussia elimination we can compute its rank as follows $$\smat{1 & 1 & 0 & \ldots & 0 \\ 0 & 1 & 1 & \ldots & 0 \\ & & \vdots & & \\ 1 & 0 & 0 & \ldots & 1} \longrightarrow \smat{1 & 1 & 0 & \ldots & 0 \\ 0 & 1 & 1 & \ldots & 0 \\ & & \vdots & & \\ 0 & -1 & 0 & \ldots & 1} \longrightarrow \smat{1 & 1 & 0 & \ldots & 0 \\ 0 & 1 & 1 & \ldots & 0 \\ & & \vdots & & \\ 0 & 0 & 1 & \ldots & 1} \longrightarrow \cdots$$ In particular, we observe that this sequence of operations can be obtained by applying the following strategy: starting from $i = 0$ in which we have the original matrix, on the $i$-th operation of the elimination
                    %
                    % \begin{itemize}
                    %     \item if $i$ is odd, subtract the $i$-th row of the matrix from the last one
                    %     \item if $i$ is even, add the $i$-th row of the matrix to the last one
                    % \end{itemize}
                    %
                    % Thanks to this procedure, only the last row will be modified at the end of the Gaussian elimination, and since we chose $\abs{V(H_i)}$ to be odd, this procedure will always end with a matrix of the following form $$\smat{1 & 1 & 0 & \ldots & 0 \\ 0 & 1 & 1 & \ldots & 0 \\ & & \vdots & & \\ 0 & 0 & 0 & \ldots & 2}$$ meaning that its rank will always be the number of rows, i.e. $\abs{V(H_i)}$. This proves that if $H_i$ is an odd-length cycle, 


                    We know that $A^{H_i}x = b$ has unique solution if and only if the rank of the matrix is equal to the number of unknowns, which is going to be $\abs{V(H_i)}$. By the rank-nullity theorem, we know that $$\mathrm{rank}(A^{H_i}) + \dim(\ker \mathscr L_{A^{H_i}}) = \abs{V(H_i)}$$ where $\mathscr L_{A^{H_i}}$ is the function that describes the \tit{left application} of $A^{H_i}$. Hence, to prove that $\mathrm{rank}(A^{H_i}) = \abs{V(H_i)}$ it suffices to show that $\dim(\ker \mathscr L_{A^{H_i}}) = 0$.

                    Assume that $H_i$ is not bipartite, and by way of contradiction suppose that there exists $\bar x = [f(e_1) \ \cdots \ f(e_k)]^T \in \ker \mathscr L_{A^{H_i}}$ such that $\bar x \neq 0$. By definition, since $\bar x$ is in the kernel of $\mathscr L_{A^{H_i}}$, it holds that $A^{H_i} \bar x = 0$, and by construction of $A^{H_i}$ this happens if and only if for each edge $\{i, j\} \in E(H_i)$ it holds that $$f(\{i\}) + f(\{j\}) = 0 \iff f(\{i\}) = - f(\{j\})$$ This means that the values assigned to adjacent vertices \tit{are always the same but opposite in sign}: in other words, the vertices of $V(H_i)$ can be 2-colored such that
                    ...
                    Vice versa, assume that $H_i$ is biparite through a bipartition $(A, B)$; we define the following function $$\forall i \in [n] \quad f(\{i\}) = \soe{ll}{1 & i \in A \\ -1 & B}$$ Fix an edge $\{i, j\} \in E(H_i)$, and without loss of generality suppose that $i \in A$ and $j \in B$; then, it holds that $$f(\{i\}) + f(\{j\}) = 1 + (- 1) = 0$$ which means that the vector $\bar x = [f(e_1) \ \cdots \ f(e_k)]^T$ is clearly inside the kernel of $\mathscr L_{A^{H_i}}$
